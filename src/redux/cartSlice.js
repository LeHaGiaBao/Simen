	...
	reducers: {
		addToCart: (state, action) => {
			...
			if (itemIndex >= 0) {
				...
				toast.info(
					`inscreased ${state.cartItems[itemIndex].name} cart quantity`,
					{
		...
		addQuantityItem: (state, action) => {
			...
			if (itemIndex >= 0) {
				toast.info(
					`inscreased ${action.payload.quantity} ${action.payload.productItem.name} cart quantity`,
					{
						position: 'bottom-left',
					}
				)
				...
				state.cartItems.push(tempProduct)
				toast.success(
					`${action.payload.quantity} ${action.payload.productItem.name} added to cart`,
					{
						position: 'bottom-left',
					}
				)
